...
pspVagina<-na.omit(unlist(psp[,c('Vaginal.swab')]))
...
pspVagina<-cacheOperation('vaginapspBayes.Rdat',BESTmcmc,pspNegs,pspVagina,burnInSteps=2e5,thinSteps=10,numSavedSteps=2e6,parallel=TRUE)
pspVaginaDiff<-pspVagina$mu1-pspVagina$mu2
...
pspVaginaTab<-table(cut(pspVaginaDiff,bins))/length(pspVaginaDiff)
...
pdf('mobioPsp.pdf')
  ...
  polygon(c(-5,bins[-length(bins)]+.05,25,-5),c(0,unlist(pspVaginaTab),0,0),col='#FF990044')
  polygon(c(-5,bins[-length(bins)]+.05,25,-5),c(0,unlist(pspOtherTab),0,0),col='#00FF0044')
  abline(v=0,lty=2)
  legend('topright',c('Placenta','Saliva','Vaginal swab','Negatives'),fill=c('#0000FF44','#FF000044','#FF550044','#00FF0044'),bty='n')
